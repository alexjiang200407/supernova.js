...
import { AnimatedVector, createCircleTexture, flipCoin, random, RGB_COLORS } from './Util'
...
export class SuperNovaAnimation {
  ...
  reset() {
    ...
    for (let i = 0; i < MAX_PTS_COUNT; i++) {
      ...
      if (i % driftFreq === 0) {
        const driftX = random(-MAX_DRIFT, MAX_DRIFT)
        ptInfo.velocity.x += driftX

        const driftY = random(-MAX_DRIFT, MAX_DRIFT)
...
export function newSupernovaAnimation(scene: SceneEx) {
  ...
  points.userData.update = (_t: number) => {
    superNova.animate()
  }
